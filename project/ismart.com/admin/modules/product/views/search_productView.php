...
#Tổng số bản ghi
$list_cat_all = db_search_all_product($value);
...
$list_product = get_product($start, $num_per_page, "CONVERT(`product_name` USING utf8) LIKE '%{$value}%' OR  CONVERT(`product_code` USING utf8) LIKE '%{$value}%'");

//product đã phê duyệt
$total_approved = db_num_rows("SELECT* FROM  `tbl_product` Where `status`= 'Approved' AND CONVERT(`product_name` USING utf8) LIKE '%{$value}%' OR  CONVERT(`product_code` USING utf8) LIKE '%{$value}%'");
//page chờ xét duyệt
$total_waiting = db_num_rows("SELECT* FROM  `tbl_product` Where `status`= 'Waitting...' AND CONVERT(`product_name` USING utf8) LIKE '%{$value}%' OR  CONVERT(`product_code` USING utf8) LIKE '%{$value}%'");
//page trash
$total_strash = db_num_rows("SELECT* FROM  `tbl_product` Where `status`= 'Trash' AND CONVERT(`product_name` USING utf8) LIKE '%{$value}%' OR  CONVERT(`product_code` USING utf8) LIKE '%{$value}%'");
?>
        ...
        <div id="content" class="fl-right">
            ...
            <div class="section" id="detail-page">
                <div class="section-detail">
                    ...
                    <form method="POST" action="?mod=product&action=apply_page&value=<?php echo $value ?>&page=<?php echo $page_num; ?>">
                        <div class="actions">
            ...
            <div class="section" id="paging-wp">
                <div class="section-detail clearfix">
                    ...
                    <ul id="list-paging-product" class="fl-right">
                        <?php
                        echo get_pagging($num_page, $page_num, "?mod=product&action=result_search&value=$value");
                        ?>
                    </ul>
                </div>
            </div>
