...
#Tổng số bản ghi
$list_post_all = db_search_all_post($value);
$total_row = count($list_post_all);
...
$list_post = get_post_paging($start, $num_per_page, "CONVERT(`post_title` USING utf8) LIKE '%{$value}%' OR  CONVERT(`creator` USING utf8) LIKE '%{$value}%'");
/*=========================================================*/
//page đã phê duyệt
$total_approved = db_num_rows("SELECT* FROM  `tbl_post` Where `post_status`= 'Approved' AND CONVERT(`post_title` USING utf8) LIKE '%{$value}%' OR  CONVERT(`creator` USING utf8) LIKE '%{$value}%'");
//page chờ xét duyệt
$total_waiting = db_num_rows("SELECT* FROM  `tbl_post` Where `post_status`= 'Waitting...' AND CONVERT(`post_title` USING utf8) LIKE '%{$value}%' OR  CONVERT(`creator` USING utf8) LIKE '%{$value}%'");
//page trash
$total_strash = db_num_rows("SELECT* FROM  `tbl_post` Where `post_status`= 'Trash' AND CONVERT(`post_title` USING utf8) LIKE '%{$value}%' OR  CONVERT(`creator` USING utf8) LIKE '%{$value}%'");
?>
        ...
        <div id="content" class="fl-right">
            ...
            <div class="section" id="detail-page">
                <div class="section-detail">
                    ...
                    <form method="POST" action="?mod=post&action=apply_page&value=<?php echo $value; ?>&page=<?php echo $page_num; ?>">
                        <div class="actions">
            ...
            <div class="section" id="paging-wp">
                <div class="section-detail clearfix">
                    ...
                    <ul id="list-paging-cat-post" class="fl-right">
                        <?php
                        echo get_pagging($num_page, $page_num, "?mod=post&action=result_search&value=$value");
                        ?>
                    </ul>
                </div>
            </div>
