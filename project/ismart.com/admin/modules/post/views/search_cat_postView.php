...
#Tổng số bản ghi
$list_cat_all = db_search_all_cat_post($value);
...
$lis_cat_post = get_cat_post($start, $num_per_page,"CONVERT(`title` USING utf8) LIKE '%{$value}%' OR  CONVERT(`cretor` USING utf8) LIKE '%{$value}%'");
/*=========================================================*/
//page đã phê duyệt
$total_approved = db_num_rows("SELECT* FROM  `tbl_post_cat` Where `cat_status`= 'Approved' AND CONVERT(`title` USING utf8) LIKE '%{$value}%' OR  CONVERT(`cretor` USING utf8) LIKE '%{$value}%'");
//page chờ xét duyệt
$total_waiting = db_num_rows("SELECT* FROM  `tbl_post_cat` Where `cat_status`= 'Waitting...' AND CONVERT(`title` USING utf8) LIKE '%{$value}%' OR  CONVERT(`cretor` USING utf8) LIKE '%{$value}%'");
//page trash
$total_strash = db_num_rows("SELECT* FROM  `tbl_post_cat` Where `cat_status`= 'Trash' AND CONVERT(`title` USING utf8) LIKE '%{$value}%' OR  CONVERT(`cretor` USING utf8) LIKE '%{$value}%'");
?>
        ...
        <div id="content" class="fl-right">
            ...
            <div class="section" id="detail-page">
                <div class="section-detail">
                    ...
                    <form method="POST" action="?mod=post&controller=cat&action=apply_page&value=<?php echo $value; ?>&page=<?php echo $page_num; ?>">
                        <div class="actions">
                                        ...
                                        foreach ($lis_cat_post as $item) {
                                            ...
                                            <tr>
                                                ...
                                                <td class="clearfix">
                                                    ...
                                                    <li><a href="?mod=post&controller=cat&action=update_cat&id_cat_post=<?php echo $item['cat_id']; ?>" title="Sửa" class="edit"><i class="fa fa-pencil" aria-hidden="true"></i></a></li>
                                                        <li><a href="?mod=post&controller=cat&action=delete_cat_post&id_cat_post=<?php echo $item['cat_id']; ?>" title="Xóa" class="delete"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                                    </ul>
                                                </td>
            ...
            <div class="section" id="paging-wp">
                <div class="section-detail clearfix">
                    ...
                    <ul id="list-paging-cat-post" class="fl-right">
                        <?php
                        echo get_pagging($num_page, $page_num, "?mod=post&controller=cat&action=result_search&value=$value");
                        ?>
                    </ul>
                </div>
            </div>
